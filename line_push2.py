...
def build_flex_card(ship):
    ...
    if zone == "12":
        ...
        header_color = "#6A1B9A"  # 🟣 紫色警戒區
        header_text = f"🚨 警戒區入侵警報 ({ship.get('alarm_name', '')})"
    else:
...
# 主推播函式（支援警戒區）
# =========================================
def send_line_alert(ships_inside, ships_outside, *, force=False, send_empty_summary=False, custom_message=None):
    ...
    if custom_message:
        try:
            ...
            print(f"📤 已發送警戒區推播: {custom_message[:30]}...")
        except Exception as e:
            print(f"❌ 自訂警戒推播失敗: {e}")
        return
    ...
    if entering_ships:
        ...
        if force or (_last_push_hash_enter != hash_enter or not _last_push_time or now - _last_push_time > PUSH_COOLDOWN):
            ...
            safe_push(LINE_TARGET_USER_ID, flex_msg)
